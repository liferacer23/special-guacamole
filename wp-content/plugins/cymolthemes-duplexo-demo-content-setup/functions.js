jQuery( document ).ready(function($) {
	...
	if (currurl.indexOf('tmdemosuccess=yes') > -1) {
		...
		window.history.pushState( {urlPath:currurl} , 'Duplexo Options < Duplexo - WordPress', currurl );
	}
	...
	jQuery('.cymolthemes-one-click-demo-content-wrapper').each(function(){
		
		var thisWrapper = jQuery(this);
		
		/*** Ajax process ***/
		jQuery('#cymolthemes_one_click_demo_content').on('click', function() {
			...
			resultDiv.addClass('cymolthemes-import-demo-progress'); /* Adding loader class */
			...
			$.ajax({
				...
				dataType: "json",
				data: {
					'action'		: 'duplexo_install_demo_data',
				...
				success: function( result ) {
					function demoInstallerStep( result ) {
						...
						if( result != null && typeof( result ) == 'object' ) {
							...
							resultDiv.append( '<p><strong style="color: red">' + duplexoVars.strError + ":</strong> " + duplexoVars.strWrongServerAnswer + '</p>' ).addClass('error');
							button.removeAttr('disabled');
