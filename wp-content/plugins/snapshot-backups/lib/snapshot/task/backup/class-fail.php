	...
	private static function get_texts( $service_error, $backup_type, $backup_id ) {
		...
		switch ( $service_error ) {
			...
			case self::ERROR_STORAGE_LIMIT:
				/* translators: %1$s - manual/scheduled backup, %2$s - Site URL, %3$s - Site domain */
				$result['p1_html'] = sprintf( __( 'The %1$s for <a href="%2$s" target="_blank">%3$s</a> failed due to insufficient storage space. The following error message is listed in the Snapshot log:', 'snapshot' ), $backup_type_html, $site_url_html, $site_host_html );
				/* translators: %s - support link */
