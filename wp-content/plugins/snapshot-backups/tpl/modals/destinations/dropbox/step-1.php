...
/**
 * @var $auth_url Dropbox authentication URL.
 */
?>
<div class="sui-modal-slide sui-loaded" id="snapshot-add-destination-dialog-slide-2-dropbox" data-modal-size="md">
	<div class="sui-box">
		...
		<div class="sui-box-body">
			...
			<div role="alert" id="snapshot-wrong-dropbox-creds" class="sui-notice sui-notice-error" aria-live="assertive" style="display: none;">
				<div class="sui-notice-content">
					<div class="sui-notice-message">
						<span class="sui-notice-icon sui-icon-info sui-md" aria-hidden="true"></span>
						<?php if ( Settings::get_branding_hide_doc_link() ) { ?>
							<p><?php esc_html_e( 'It appears the authorization process went wrong. Please try again by clicking the "Authenticate" button and make sure you authorize Snapshot to access your Dropbox. If you run into further issues, you can contact our Support team for help.', 'snapshot' ); ?></p>
						<?php } else { ?>
							<?php /* translators: %s - Link for support */ ?>
							<p><?php echo wp_kses_post( sprintf( __('It appears the authorization process went wrong. Please try again by clicking the "Authenticate" button and make sure you authorize Snapshot to access your Dropbox. If you run into further issues, you can <a href="%s" target="_blank">contact our Support</a> team for help.', 'snapshot' ), Task\Backup\Fail::URL_CONTACT_SUPPORT ) ); ?></p>
						<?php } ?>
					</div>
				</div>
			</div>

			<div role="alert" id="snapshot-correct-dropbox-creds" class="sui-notice sui-notice-success" aria-live="assertive" style="display: none;">
				<div class="sui-notice-content">
					<div class="sui-notice-message">
						<span class="sui-notice-icon sui-icon-info sui-md" aria-hidden="true"></span>
						<p><?php echo wp_kses_post( __( 'Snapshot has been successfully authorized in Dropbox.', 'snapshot' ) ); ?></p>
					</div>
				</div>
			</div>
			...
			<div id="snapshot-dropbox-authorization">
				...
				<div class="sui-border-frame">
					<form method="post" id="snapshot-test-dropbox-connection">
						<input type="hidden" id="_wpnonce-snapshot_dropbox_connection" name="_wpnonce-snapshot_dropbox_connection" value="">
					...
					<p class="sui-description"><?php echo wp_kses_post( __( 'Connect with Dropbox integration by authenticating it using the button below. Note that you\'ll be taken to the Dropbox website to grant access to Snapshot and then redirected back.', 'snapshot' ) ); ?></p>


					<a type="button" href="<?php echo esc_url( $auth_url ); ?>" class="sui-button sui-button-lg snapshot-connect-dropbox">
		...
		<div class="sui-box-footer sui-flatten sui-lg sui-content-separated">
			...
			<button class="sui-button sui-button-icon-right snapshot-next-destination-screen" style="display:none;" id="snapshot-submit-dropbox-generate-tokens">
