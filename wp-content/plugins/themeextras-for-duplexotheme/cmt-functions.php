			...
			switch( $meta_id ){
				...
				case 'author':
					...
					if ( 'post' === get_post_type() ) {
						...
						$return .= sprintf( '<span class="cmt-meta-line byline">' .get_avatar( get_the_author_meta( 'ID' ), 32 ) . '<span class="author vcard"><span class="screen-reader-text cmt-hide">%1$s </span>%2$s</span></span>',
				...
				case 'comment':
					...
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
						$return .= '<span class="cmt-meta-line comments-link"><i class="cmt-duplexo-icon-comment-3"></i> ';
						ob_start();
...
function cymolthemes_portfoliobox_media_link( $default_icon='cmt-duplexo-icon-search', $video_icon='cmt-duplexo-icon-video', $audio_icon='cmt-duplexo-icon-music-alt', $slider_icon='cmt-duplexo-icon-gallery-1' ){
...
function cymolthemes_blogbox_readmore(){
	...
	if( strpos(get_the_content(), '"more-link"')!==false && get_post_format()!='quote' && get_post_format()!='link' ) {
		$return .= '<div class="cymolthemes-blogbox-footer-left cymolthemes-wrap-cell">';
...
if ( !function_exists( 'cymolthemes_servicebox_icon1' ) ){
function cymolthemes_servicebox_icon1( $post_id='' ){
	$return = '';
	$box_icon = get_post_meta( get_the_ID(), 'cymolthemes_tmservice_icon', true);
	...
	if( !empty($box_icon['cmt_serviceicon']) ){	
		$servicebox_icon = cymolthemes_create_icon_from_data( $box_icon['cmt_serviceicon'], true );
		if( !empty($servicebox_icon) ){
			$return .= '<div class="cmt-service-iconbox"><div class="cmt-service-icon-dots"></div><span class="cmt-service-icon">' . $servicebox_icon . '</span></div>';
		}
	}
	return $return;
}
...
function cymolthemes_topbar_inline_style(){
	...
	if( $topbar_bg_color=='custom' ){
		$return .= '.cymolthemes-topbar-wrapper{background-color:'. $topbar_bg_custom_color .';}';
	}
	
	// Text Color CSS code
	if( $topbar_text_color=='custom' ){
		$return .= '.cymolthemes-topbar-wrapper, .cymolthemes-topbar-wrapper a{color:'. $topbar_text_custom_color .';}';
	}
...
function cymolthemes_header_links(){
	...
	if( $header_search==true ){
		$class   = '';
		$return .= '<div class="cmt-header-icon cmt-sboxheader-search-link"><a href="#"><i class="cmt-duplexo-icon-search"></i></a></div>';
	}
...
function cymolthemes_header_class( $extra_class='' ){
	...
	if( !empty($sticky_header_bg_color) ) {
		$class[] = 'cmt-sboxsticky-bgcolor-'.esc_attr( $sticky_header_bg_color );
	};
...
function cymolthemes_header_style_class( $echo=false ){
	...
	$return     .= ' cmt-sboxheader-style-'. $headerstyle;

	if (strpos( cymolthemes_get_headerstyle(), 'two') !== false) {
		$return .= ' cmt-sboxheader-two';
	}
...
function cymolthemes_header_menu_class(){
	...
	if( !empty($header_menu_bg_color) ){
		$class .= ' cmt-sboxheader-menu-bg-color-'. sanitize_html_class($header_menu_bg_color) .' cmt-bgcolor-'. sanitize_html_class($header_menu_bg_color);
	}
	
	// sticky class
	if( !empty($sticky_header_menu_bg_color) ){
		$class .= ' cmt-sboxsticky-bgcolor-'. sanitize_html_class($sticky_header_menu_bg_color);
	}
...
if( !function_exists('cymolthemes_search_form') ){
function cymolthemes_search_form(){
	$return = '';
	
	
	$cptList = array(
		'any'            => esc_attr__('All selections', 'duplexo'),
		'page'           => esc_attr__('Pages', 'duplexo'),
		'post'           => esc_attr__('Blog posts', 'duplexo'),
	);
	
	$cpt_obj_portfolio   = get_post_type_object( 'cmt_portfolio' );
	if( !empty($cpt_obj_portfolio->label) ){
		$cptList['cmt_portfolio'] = esc_attr($cpt_obj_portfolio->label);
	}
	
	$cpt_obj_team   = get_post_type_object( 'cmt_team_member' );
	if( !empty($cpt_obj_team->label) ){
		$cptList['cmt_team_member'] = esc_attr($cpt_obj_team->label);
	}
	...
	// CPT Dropdown
	$dropdown = '<select class="cmt-sboxsresult-cpt-select">';
	...
	// Form
	$return .= '
	<div class="cmt-sboxsresult-form-wrapper">
					<div class="cmt-sboxsresult-form-top">
						<h2>
							<i class="fa fa-search"></i>
							' . esc_attr__('You searched for', 'duplexo') . '
						</h2>
						' . get_search_form(false) . '
						<div class="cmt-sboxsresults-settings-wrapper">
							<a class="cmt-sboxsresults-settings-btn" href="#">
								<i class="fa fa-gear"></i>  
								<span>' . esc_attr__('Settings', 'duplexo') . '</span>
							</a>
						</div>
						<div class="clr clear"></div>
					</div>
					<div class="cmt-sboxsresult-form-bottom-w">
						<div class="cmt-sboxsresult-form-bottom row" style="display:none;">
						<div class="cmt-sboxsearch-main-box clearfix">
                            <div class="cmt-sboxsearch-text"><strong>' . esc_attr__('Search in:','duplexo') . '</strong></div>
							<div class="cmt-sboxsearch-select-box">
								' . $dropdown . '
                              <div class="cmt-sboxsresult-form-sbtbtn-wrapper">
								<input class="cmt-sboxsresult-form-sbtbtn" type="submit" value="' . esc_attr__('Search now','duplexo') . '" />
							  </div>
						  </div>
							
                          </div>
						</div><!-- .cmt-sboxsresult-form-bottom -->
					</div><!-- .cmt-sboxsresult-form-bottom-w -->
				</div>
...
function cymolthemes_footer_copyright_right(){
	...
	if ( !shortcode_exists( 'cmt-footermenu' ) ) {
		$right = str_replace('[cmt-footermenu]', '', $right );
	}
