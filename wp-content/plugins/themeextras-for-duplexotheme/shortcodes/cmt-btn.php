...
if( !function_exists('cymolthemes_sc_btn') ){
function cymolthemes_sc_btn( $atts, $content=NULL ) {
	...
	if( function_exists('vc_map') ){
		...
		$options_list   = cymolthemes_create_options_list($cmt_sc_params_btn);
		...
		//parse link
		$link = ( '||' === $link ) ? '' : $link;
		$link = cymolthemes_vc_build_link( $link );
		$use_link = false;
		...
		// CSS classes in DIV
		$css_class   = array();
		$css_class[] = 'cmt-vc_btn3-container';
		...
		if( !empty($css_animation) ){
			$css_animation = cymolthemes_getCSSAnimation( $css_animation );
		...
		$wrapper_classes = array(
			...
			cymolthemes_getExtraClass( $el_class ),
			cymolthemes_getCSSAnimation( $css_animation ),
			'cmt-vc_btn3-' . $align,
		);
		...
		$button_classes = array(
			'cmt-vc_general',
			'cmt-vc_btn3',
			'cmt-vc_btn3-size-' . $size,
			'cmt-vc_btn3-shape-' . $shape,
			'cmt-vc_btn3-style-' . $style,
			'cmt-vc_btn3-weight-' . $font_weight,
		);
		
		
		// Text style - adding outline class to set all text color
		if( $style=='text' ){
			//$button_classes[] = 'cmt-vc_btn3-style-outline';
		}
		...
		$button_html = $title;
		if ( '' === trim( $title ) ) {
			$button_classes[] = 'cmt-vc_btn3-o-empty';
			$button_html = '<span class="cmt-vc_btn3-placeholder">&nbsp;</span>';
		}
		
		
		
		if ( 'true' === $button_block && 'inline' !== $align ) {
			$button_classes[] = 'cmt-vc_btn3-block';
		}
		if ( 'true' === $add_icon ) {
			$button_classes[] = 'cmt-vc_btn3-icon-' . $i_align;
			cymolthemes_vc_icon_element_fonts_enqueue( $i_type );
			...
			if ( $icon_wrapper==true ) {
				$icon_html = '<i class="cmt-vc_btn3-icon"><span class="cmt-vc_btn3-icon-inner ' . esc_attr( $icon_class ) . '"></span></i>';
			} else {
				$icon_html = '<i class="cmt-vc_btn3-icon ' . esc_attr( $icon_class ) . '"></i>';
			}
		...
		if ( 'custom' === $style ) {
			if ( $custom_background ) {
				$styles[] = cmt_vc_get_css_color( 'background-color', $custom_background );
			}

			if ( $custom_text ) {
				$styles[] = cmt_vc_get_css_color( 'color', $custom_text );
			}

			if ( ! $custom_background && ! $custom_text ) {
				$button_classes[] = 'cmt-vc_btn3-color-grey';
			}
		} elseif ( 'outline-custom' === $style ) {
			if ( $outline_custom_color ) {
				$styles[] = cmt_vc_get_css_color( 'border-color', $outline_custom_color );
				$styles[] = cmt_vc_get_css_color( 'color', $outline_custom_color );
			...
			if ( ! $outline_custom_color && ! $outline_custom_hover_background && ! $outline_custom_hover_text ) {
				$button_classes[] = 'cmt-vc_btn3-color-inverse';

				foreach ( $button_classes as $k => $v ) {
					if ( 'cmt-vc_btn3-style-outline-custom' === $v ) {
			...
			$uid = uniqid();
			$return .= '<style type="text/css">.cmt-vc_btn3-style-' . $style . '.cmt-sboxvc_btn-gradient-btn-' . $uid . ':hover{' . implode( ';',
					$gradient_css_hover ) . ';' . '}</style>';
			$return .= '<style type="text/css">.cmt-vc_btn3-style-' . $style . '.cmt-sboxvc_btn-gradient-btn-' . $uid . '{' . implode( ';',
					$gradient_css ) . ';' . '}</style>';
			$button_classes[] = 'cmt-sboxvc_btn-gradient-btn-' . $uid;
			$attributes[] = 'data-vc-gradient-1="' . $gradient_color_1 . '"';
			$attributes[] = 'data-vc-gradient-2="' . $gradient_color_2 . '"';
		} else {
			$button_classes[] = 'cmt-vc_btn3-color-' . $color;
		}
...
add_shortcode( 'cmt-sboxbtn', 'cymolthemes_sc_btn' );