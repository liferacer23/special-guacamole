<?php
// [cmt-custom-heading tag="h1" text="This is heading text"]
if( !function_exists('cymolthemes_sc_custom_heading') ){
function cymolthemes_sc_custom_heading( $atts, $content=NULL ){
	...
	if( function_exists('vc_map') ){
		...
		$options_list = cymolthemes_create_options_list($cmt_sc_params_custom_heading);
		...
		$inline_style = '';
		$css_class    = 'cmt-custom-heading';
		
		// converting vc value to array
		$font_container_data = cymolthemes_vc_parse_multi_attribute(
			$font_container,
			array(
				'tag'         => 'h2',
				'text_align'  => '',
		...
		$google_fonts_data   = cymolthemes_vc_parse_multi_attribute(
			$google_fonts,
			array(
				'font_family' => '',
				'font_style'  => '',
		...
		if( !empty($css_class) ){
			...
			$design_option_class = cymolthemes_vc_shortcode_custom_css_class( $css );
		...
		if( !empty($alldata['font_family']) ){
			...
			cymolthemes_footer_google_fonts_array( $font_family , $font_weight );
		}
...
add_shortcode( 'cmt-custom-heading', 'cymolthemes_sc_custom_heading' );