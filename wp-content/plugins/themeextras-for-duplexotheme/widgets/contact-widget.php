 ...
 * @since 1.0
 */
class duplexo_contact_widget extends WP_Widget {
	...
	function __construct() {
		$widget_style = array('classname'   => 'duplexo_contact_widget',
							  'description' => esc_attr__('Show Contact details with icons.', 'duplexo') );
							  
		$widget_define = array('show_id'   => 'cymolthemes_single_contact',
							   'get_tips'  => 'true',
							   'get_title' => 'true');
							   
		$control_styles = array('width'   => 300,
								'height'  => 350,
								'id_base' => 'duplexo_contact_widget');
		...
		parent::__construct(
			'duplexo_contact_widget', // Base ID
			esc_attr__('Duplexo Contact Widget', 'duplexo'), // Name
			$widget_style // Args
		);
	}
	...
	function widget( $args, $cur_instance ) {
		...
		if ( function_exists( 'icl_register_string' ) ) {
			icl_register_string( 'Duplexo Contact Widget', 'Phone Number' . $this->id, $Phone );
		}
		if ( function_exists( 'icl_t' ) ) {
			$Phone = icl_t( 'Duplexo Contact Widget', 'Phone Number' . $this->id, $Phone );
		}
		
		// Email
		if ( function_exists( 'icl_register_string' ) ) {
			icl_register_string( 'Duplexo Contact Widget', 'Email Address' . $this->id, $Email );
		}
		if ( function_exists( 'icl_t' ) ) {
			$Email = icl_t( 'Duplexo Contact Widget', 'Email Address' . $this->id, $Email );
		}
		
		// Website
		if ( function_exists( 'icl_register_string' ) ) {
			icl_register_string( 'Duplexo Contact Widget', 'Website URL' . $this->id, $Website );
		}
		if ( function_exists( 'icl_t' ) ) {
			$Website = icl_t( 'Duplexo Contact Widget', 'Website URL' . $this->id, $Website );
		}
		
		// Address
		if ( function_exists( 'icl_register_string' ) ) {
			icl_register_string( 'Duplexo Contact Widget', 'Address' . $this->id, $Address );
		}
		if ( function_exists( 'icl_t' ) ) {
			$Address = icl_t( 'Duplexo Contact Widget', 'Address' . $this->id, $Address );
		}
		
		// Time
		if ( function_exists( 'icl_register_string' ) ) {
			icl_register_string( 'Duplexo Contact Widget', 'Time' . $this->id, $Time );
		}
		if ( function_exists( 'icl_t' ) ) {
			$Time = icl_t( 'Duplexo Contact Widget', 'Time' . $this->id, $Time );
		}
		...
		<ul class="duplexo_contact_widget_wrapper">
			<?php if( trim($Address)!='' ): ?><li class="cymolthemes-contact-address cmt-duplexo-icon-location">
			<?php 
				echo wp_kses( /* HTML Filter */
					nl2br($Address),
					array(
			...
			<?php if( trim($Email)!='' ): ?><li class="cymolthemes-contact-email cmt-duplexo-icon-mail"><?php echo '<a href="mailto:'.sanitize_email($Email).'" target="_blank">'.sanitize_email($Email).'</a>'; ?></li><?php endif; ?>
			<?php if( trim($Phone)!='' ): ?><li class="cymolthemes-contact-phonenumber cmt-duplexo-icon-phone">
			<?php 
				echo wp_kses( /* HTML Filter */
					nl2br($Phone),
					array(
			...
			<?php if( trim($Website)!='' ): ?><li class="cymolthemes-contact-website  cmt-duplexo-icon-world"><?php echo '<a href="'.esc_url(duplexo_addhttp($Website)).'" target="_blank">'.esc_url($Website).'</a>'; ?></li><?php endif; ?>
			<?php if( trim($Time)!='' ): ?><li class="cymolthemes-contact-time cmt-duplexo-icon-clock">
			<?php 
				echo wp_kses( /* HTML Filter */
					nl2br($Time),
					array(
	...
	function form( $cur_instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'Phone' )); ?>"><?php esc_attr_e('Phone', 'duplexo'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'Email' )); ?>"><?php esc_attr_e('Email', 'duplexo'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'Website' )); ?>"><?php esc_attr_e('Website', 'duplexo'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'Address' )); ?>"><?php esc_attr_e('Address', 'duplexo'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'Time' )); ?>"><?php esc_attr_e('Time', 'duplexo'); ?>:</label>
...
register_widget( 'duplexo_contact_widget' );