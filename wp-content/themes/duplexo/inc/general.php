 ...
 */
if( !function_exists('cymolthemes_megamenu_override_default_theme') ){
function cymolthemes_megamenu_override_default_theme($value) {
  // change 'cymolthemes-main-menu' to your menu location ID
  if ( !isset($value['cymolthemes-main-menu']['theme']) ) {
    $value['cymolthemes-main-menu']['theme'] = 'duplexo_theme_settings'; // change my_custom_theme_key to the ID of your exported theme
  }

  return $value;
}
}
add_filter('default_option_megamenu_settings', 'cymolthemes_megamenu_override_default_theme');


/**
 *  Updates Responsive Breakpoint in Max Mega Menu plugin 
 */
if( !function_exists('cymolthemes_mmmenu_breakpoint') ){
function cymolthemes_mmmenu_breakpoint( $options ){
	...
	if( empty($megamenu_themes) ){
		$megamenu_themes = array (
			'duplexo_theme_settings' => 
				array (
				'title' => 'Duplexo Theme Settings',
				'arrow_up' => 'dash-f343',
...
add_filter( 'cs_validate_save', 'cymolthemes_mmmenu_breakpoint' ); // On theme options save.. for CodeStar Framework only
...
function cymolthemes_pf_reset_like(){
    $screen = get_current_screen();
    if ( $screen->post_type == 'cmt_portfolio' && isset($_GET['action']) && $_GET['action']=='edit' && !isset($_GET['taxonomy']) ){
        global $post;
        $postID = $_GET['post'];
        $resetVal = get_post_meta($postID, 'cymolthemes_portfolio_like' ,true );

        if( $resetVal==true ){
            // Do reset processs now
            update_post_meta($postID, 'cymolthemes_likes' , '0' ); // Setting ZERO
            update_post_meta($postID, 'cymolthemes_portfolio_like' ,'' ); // Removing checkbox
        }
    }
    
}
add_action('current_screen', 'cymolthemes_pf_reset_like');
...
if( !function_exists('cymolthemes_image_sizes') ){
function cymolthemes_image_sizes(){
	
	$img_size_blog        = cymolthemes_get_option('img-size-blog');
	$img_size_blog_left   = cymolthemes_get_option('img-size-blog-left');
	$img_size_blog_top   = cymolthemes_get_option('img-size-blog-top');
	$img_size_portfolio   = cymolthemes_get_option('img-size-portfolio');
	$img_size_team_member = cymolthemes_get_option('img-size-team-member');
	...
	add_image_size( 'cymolthemes-img-blog', $img_size_blog['width'], $img_size_blog['height'], $img_size_blog_crop ); // (cropped)
	add_image_size( 'cymolthemes-img-blog-left', $img_size_blog_left['width'], $img_size_blog_left['height'], $img_size_blog_left_crop ); // (cropped)
	add_image_size( 'cymolthemes-img-blog-top', $img_size_blog_top['width'], $img_size_blog_top['height'], $img_size_blog_left_crop ); // (cropped)
	add_image_size( 'cymolthemes-img-portfolio', $img_size_portfolio['width'], $img_size_portfolio['height'], $img_size_portfolio_crop ); // (cropped)
	add_image_size( 'cymolthemes-img-team-member', $img_size_team_member['width'], $img_size_team_member['height'], $img_size_team_member_crop ); // (cropped)

	add_image_size( 'cymolthemes-img-portfolio-style2', 600, 700, true);
	add_image_size( 'cymolthemes-img-size-two', 700, 466, true);
	add_image_size( 'cymolthemes-img-blog-stylethree', 570, 327, true);
}
}
add_action( 'init', 'cymolthemes_image_sizes' );


/**
 *  Wrap "Read More" link with some div so we can design it
 */
if( !function_exists('cymolthemes_wrap_more_link') ){
function cymolthemes_wrap_more_link($more) {
	return '<span class="more-link-wrapper">'.cymolthemes_wp_kses($more).'</span>';
}
}
add_filter('the_content_more_link','cymolthemes_wrap_more_link');


// Slider Revoluiton Theme integration
add_action( 'init', 'cymolthemes_set_rs_as_theme' );
function cymolthemes_set_rs_as_theme() {
 ...
 */
if( !function_exists('cymolthemes_single_skin_color') ){
function cymolthemes_single_skin_color(){
	...
	if( $post_id ){
		...
		$page_meta = get_post_meta( $post_id, '_cymolthemes_metabox_group', true );
		if( !empty($page_meta['skincolor']) ){
			$duplexo_theme_options['skincolor'] = esc_attr($page_meta['skincolor']);
		}
	}
}
}
add_action('wp','cymolthemes_single_skin_color');


/**
 *  Override Theme Options value from single page/post/cpt. This is useful for demo purpose and for other users too.
 */
if( !function_exists('cymolthemes_toptions_override') ){
function cymolthemes_toptions_override(){
			...
			foreach( $page_meta as $meta=>$value ){
				...
				if( substr($meta, 0, strlen($prefix) ) == $prefix ){
					...
					if( in_array($meta_name, $all_options) ){
						if( cymolthemes_is_json($value[0]) && !is_numeric($value[0]) ){
							// array
							$final_val    = json_decode($value[0]);
							$final_val    = (array) $final_val;
							$original_val = ( isset($duplexo_theme_options[$meta_name]) ) ? $duplexo_theme_options[$meta_name] : array() ;
							$final_val    = array_merge( $original_val, $final_val );
						} else {
							// string
							$final_val = $value[0];
						}
						$duplexo_theme_options[$meta_name] = $final_val;
					}
...
add_action('wp','cymolthemes_toptions_override');

/**
 *  Checking if Json code in the string
 */
if( !function_exists('cymolthemes_is_json') ){
function cymolthemes_is_json($string='') {
	json_decode($string);
 ...
 */
add_action( 'wp_footer', 'cymolthemes_analytics_code' );
if( !function_exists('cymolthemes_analytics_code') ){
function cymolthemes_analytics_code(){
	
	// Custom JS code
	$custom_js_code = cymolthemes_get_option('custom_js_code');
	
	// Google Analytics code
	$customhtml_bodyend = cymolthemes_get_option('customhtml_bodyend');
 ...
 *  Custom Google Analytics code in footer
 */
add_action( 'wp_head', 'cymolthemes_inline_css_header_code' );
if( !function_exists('cymolthemes_inline_css_header_code') ){
function cymolthemes_inline_css_header_code(){
	...
	if( !empty($cymolthemes_inline_css) ){
		echo '<style>';
		echo trim($cymolthemes_inline_css);
		echo '</style>';
	}
	echo '<!-- Inline CSS End -->';		
}
 ...
 */
add_action( 'wp_head', 'cymolthemes_head_code' );
if( !function_exists('cymolthemes_head_code') ){
function cymolthemes_head_code(){
	
	// Google Analytics code
	$customhtml_head = cymolthemes_get_option('customhtml_head');
...
/**
 * Flush out the transients used in {@see duplexo_categorized_blog()}.
 *
 * @since Duplexo 1.0
 */
if( !function_exists('duplexo_category_transient_flusher') ){
function duplexo_category_transient_flusher() {
	delete_transient( 'duplexo_categories' );
}
}
add_action( 'edit_category', 'duplexo_category_transient_flusher' );
add_action( 'save_post',     'duplexo_category_transient_flusher' );


/**
 *  Add inline Dynamic Style code
 */
if( !function_exists('cymolthemes_add_inline_dynamic_css') ){
function cymolthemes_add_inline_dynamic_css(){
	global $duplexo_theme_options;
	$css = '';
	...
	$css .= cymolthemes_topbar_inline_style();
	
	// Titlebar custom css stylesheet
	if( $post_id ){
		$css .= cymolthemes_titlebar_inline_style();
	}
	
	// Minify CSS style
	if( !empty( $duplexo_theme_options['minify'] ) && esc_attr($duplexo_theme_options['minify'])==true ){
		$css = cymolthemes_minify_css( $css );
	}
	
	// duplexo core style
	wp_add_inline_style( 'duplexo-core-style', $css );
	
}
}
add_action( 'wp_enqueue_scripts', 'cymolthemes_add_inline_dynamic_css', 17 );


/*
 *  Dynamic content linking with JS code. Declaring variables.
 */
add_action('wp_head','cymolthemes_js_vars');
function cymolthemes_js_vars(){
	$breakpoint = cymolthemes_get_option('menu_breakpoint');
	$breakpoint_custom = cymolthemes_get_option('menu_breakpoint-custom');
	$breakpoint = ( $breakpoint=='custom' ) ? $breakpoint_custom : $breakpoint ;
	?>
	
	<script>
		var cmt_breakpoint = <?php echo esc_attr($breakpoint); ?>;
	</script>
...
/**
 *  This will add Google Fonts line in footer processing the $cmt_global_footer_gfonts variable
 */
if( !function_exists('cymolthemes_footer_google_fonts') ){
function cymolthemes_footer_google_fonts( $font_family, $font_weight='normal' ){
	global $cmt_global_footer_gfonts;

	if( !empty($cmt_global_footer_gfonts) ){
		
		$fontline = array();
		if( is_array($cmt_global_footer_gfonts) && count($cmt_global_footer_gfonts)>0 ){
			foreach($cmt_global_footer_gfonts as $gfonts=>$weight){
		...
		wp_enqueue_style(  'cymolthemes-footer-gfonts',  '//fonts.googleapis.com/css?family='.$fontline );
		
	}
	
}
}
add_action( 'wp_footer', 'cymolthemes_footer_google_fonts' );


/**
 *  Single: Body Background
 */
if( !function_exists('cymolthemes_single_body_background') ){
function cymolthemes_single_body_background(){
	$css = '';
	...
	if( $post_id ){
		$single_meta = get_post_meta( $post_id, '_cymolthemes_metabox_group' , true );
		...
		if( isset($single_meta['custom_background_switcher']) && $single_meta['custom_background_switcher']==true && !empty($single_meta['custom_background']) ){
			$css = cymolthemes_get_background_css( 'body', $single_meta['custom_background'], array('output_bglayer') );
			// Add CSS code in page
			wp_add_inline_style( 'theme-last-checkpoint', $css );
		}
		if( isset($single_meta['custom_background_switcher']) && $single_meta['custom_background_switcher']==true && !empty($single_meta['custom_inner_background']) ){
			$css = cymolthemes_get_background_css( 'body #main', $single_meta['custom_inner_background'], array('output_bglayer') );
			// Add CSS code in page
			wp_add_inline_style( 'theme-last-checkpoint', $css );
		}
	}
...
add_action( 'wp_enqueue_scripts', 'cymolthemes_single_body_background', 18 );


if ( ! function_exists( 'duplexo_excerpt_more' ) && ! is_admin() ) :
/**
 ...
 */
function duplexo_excerpt_more( $more ) {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
		sprintf( esc_attr__( 'Continue reading %s', 'duplexo' ), '<span class="screen-reader-text">' . get_the_title( get_the_ID() ) . '</span>' )
		);
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'duplexo_excerpt_more' );
endif;
 ...
 */
if( !function_exists('cymolthemes_body_classes') ){
function cymolthemes_body_classes($bodyClass){
	global $duplexo_theme_options;
	$hClass = '';
	
	// All ROW to 20px 
	if( is_singular() ){
		$singular_meta = get_post_meta( get_the_ID(), 'cymolthemes_page_row_settings', true );
		if( isset($singular_meta['row_lower_padding']) && $singular_meta['row_lower_padding']==true ){
			$bodyClass[] = 'cmt-sboxall-row-20px';
		}
	}
	
	// Breadcrumb position in Titlebar
	if( isset($duplexo_theme_options['titlebar_view']) && 
		($duplexo_theme_options['titlebar_view'] == 'default' || $duplexo_theme_options['titlebar_view'] == 'allleft' || $duplexo_theme_options['titlebar_view'] == 'allright' ) &&
		isset($duplexo_theme_options['breadcrumb_on_bottom']) &&
		$duplexo_theme_options['breadcrumb_on_bottom'] == true
	){
		$bodyClass[] = 'cmt-titlebar-bcrumb-bottom';
	}
	
	
	// check if dark background set for container.
	if( !empty($duplexo_theme_options['headerstyle']) ){
		$bodyClass[] = 'headerstyle-'.esc_attr($duplexo_theme_options['headerstyle']);
	}
	
	// check if dark background set for container.
	if( isset($duplexo_theme_options['inner_background']['background-color']) && trim($duplexo_theme_options['inner_background']['background-color'])!='' && cymolthemes_check_dark_color(esc_attr($duplexo_theme_options['inner_background']['background-color'])) ){
		$bodyClass[] = 'cmt-sboxdark-layout';
	}
	
	// show/hide separator line between links in dropdown menu
	if( isset($duplexo_theme_options['dropdown_menu_separator']) && trim($duplexo_theme_options['dropdown_menu_separator'])=='0' ){
		$bodyClass[] = 'cmt-sboxdropmenu-hide-sepline';
	}
	

	// Sticky Fotoer ON/OFF
	if( isset($duplexo_theme_options['stickyfooter']) && $duplexo_theme_options['stickyfooter']==true ){
		$bodyClass[] = 'cymolthemes-sticky-footer';
	}
	
	// Single Portfolio
	if( is_singular('portfolio') ){
		$portfolioView        = esc_attr($duplexo_theme_options['portfolio_viewstyle']); // Global view
		$portfolioView_single = esc_attr(get_post_meta( get_the_ID(), '_cymolthemes_portfolio_view_viewstyle', true)); // Single portfolio view
		...
		$bodyClass[] = sanitize_html_class('cymolthemes-portfolio-view-'.$portfolioView);
	}	
	
	if( isset($duplexo_theme_options['footer_cta_box']) && $duplexo_theme_options['footer_cta_box']==true ){
		$bodyClass[] = sanitize_html_class('cymolthemes-footer-cta-yes');
	}
	
	// Boxed / Wide
	if( isset($duplexo_theme_options['layout']) && trim($duplexo_theme_options['layout'])!='' ){
		if( $duplexo_theme_options['layout']=='boxed' || $duplexo_theme_options['layout']=='framed' || $duplexo_theme_options['layout']=='rounded' ){
			$bodyClass[] = 'cymolthemes-boxed';
		}
		if( $duplexo_theme_options['layout']=='framed' || $duplexo_theme_options['layout']=='rounded' ){
			$bodyClass[] = 'cymolthemes-boxed-'.sanitize_html_class($duplexo_theme_options['layout']);
		}
		
		$bodyClass[] = sanitize_html_class( 'cymolthemes-'.trim($duplexo_theme_options['layout']));
		if( $duplexo_theme_options['layout']=='fullwide' ){
			if( isset($duplexo_theme_options['full_wide_elements']['content']) && $duplexo_theme_options['full_wide_elements']['content']=='1' ){
				$bodyClass[] = 'cmt-sboxlayout-container-full';
			}
		}
		
	} else {
		$bodyClass[] = 'cymolthemes-wide';
	}
	
	$cymolthemes_retina_logo = 'off';
	if( isset($duplexo_theme_options['logoimg_retina']['url']) && $duplexo_theme_options['logoimg_retina']['url']!=''){
		$cymolthemes_retina_logo = 'on';
	}
	...
	$headerstyle_global = '';
	$headerstyle_page   = '';
	if( isset($duplexo_theme_options['headerstyle']) && trim($duplexo_theme_options['headerstyle'])!='' ){
		$headerstyle_global = sanitize_html_class($duplexo_theme_options['headerstyle']);
	}
	if( is_page() ){
	...
	switch( $headerstyle ){
		...
		case '3':
		case '9':
			if( $headerstyle=='9' ){ $headerstyle='1 header-invert'; }
			$hClass = 'cymolthemes-header-style-'.trim($headerstyle);
			break;
		case '4':
		case '10':
			$overlayClass = ' header-two';
			if( $headerstyle=='10' ){ $overlayClass.=' header-invert'; }
			if( is_page() ){
				global $post;
				$slidertype   = get_post_meta( $post->ID, '_cymolthemes_page_options_slidertype', true );
				if( is_array($slidertype) ){ $slidertype = $slidertype[0];}
				$hidetitlebar = get_post_meta( $post->ID, '_cymolthemes_page_options_hidetitlebar', true );
				
				if( trim($slidertype)=='' && $hidetitlebar=='on' ){
					$overlayClass = '';
				}
			}
			$hClass = 'cymolthemes-header-style-1' . $overlayClass;
			break;
		case '5':
			$overlayClass = ' header-two';
			if( is_page() ){
			...
			$hClass = 'cymolthemes-header-style-2' . $overlayClass;
			break;
		case '6':
			$overlayClass = ' header-two';
			if( is_page() ){
			...
			$hClass = 'cymolthemes-header-style-3' . $overlayClass;
			break;
		case '7':
		case '8':
			$overlayClass = ' cmt-sboxheader-two';
			if( $headerstyle=='8' ){ $overlayClass.='header-invert'; }
			if( is_page() ){
				global $post;
				$slidertype   = get_post_meta( $post->ID, '_cymolthemes_page_options_slidertype', true );
				if( is_array($slidertype) ){ $slidertype = $slidertype[0];}
			...
			$hClass = 'cymolthemes-header-style-4' . $overlayClass;
			break;
	}
	...
	$sidebar = cymolthemes_get_sidebar_info();
	if( $sidebar=='' || $sidebar=='no' ){
		// The page is full width
		$bodyClass[] = 'cymolthemes-page-full-width';
	} else {
		// Sidebar class for border
		$bodyClass[] = sanitize_html_class( 'cymolthemes-sidebar-true' );
		$bodyClass[] = sanitize_html_class( 'cymolthemes-sidebar-'.$sidebar );
	}
		
	// Check if empty sidebar.. so we can add class in body to make the content area center and cover the sidebar area.
	$cymolthemes_check_empty_sidebar = cymolthemes_get_sidebar_info( 'count_widgets' );
	if( empty($cymolthemes_check_empty_sidebar) ){
		$bodyClass[] = 'cmt-sboxempty-sidebar';
	}
			
	// Check if "Max Mega Menu" plugin is active
	if ( class_exists( 'Mega_Menu' ) ) {
		// plugin is activated
		$bodyClass[] = 'cymolthemes-maxmegamenu-active';
	}
	
	// One Page website
	if( isset($duplexo_theme_options['one_page_site']) && $duplexo_theme_options['one_page_site']==true ){
		$bodyClass[] = 'cymolthemes-one-page-site';
	}
	
	return $bodyClass;
}
}
add_filter('body_class', 'cymolthemes_body_classes');


/*
 *  This is under construction message code
 */
if( !function_exists('cymolthemes_uconstruction') ){
function cymolthemes_uconstruction(){
	
	$uconstruction = cymolthemes_get_option('uconstruction');
	
	if (!is_user_logged_in() && !is_admin() && isset($uconstruction) && esc_attr($uconstruction) == true ){
		
		$uconstruction_html     = do_shortcode( cymolthemes_get_option('uconstruction_html') );
		$uconstruction_title    = do_shortcode( cymolthemes_get_option('uconstruction_title') );
		$uconstruction_css_code = cymolthemes_get_option('uconstruction_css_code');
		$uconstruction_head     = '';
		...
		// Background CSS
		$value = cymolthemes_get_option('uconstruction_background'); // not escaping as value is array
		$css   = '';
...
add_action('template_redirect', 'cymolthemes_uconstruction');


/**
 * set span tag for post count in Category widget
 */
if( !function_exists('cymolthemes_add_cat_span') ){
function cymolthemes_add_cat_span($catg) {
...
add_filter('wp_list_categories', 'cymolthemes_add_cat_span');


/**
 * Setting limit to show number of Portfolios on Portfolio Category page
 */
if( !function_exists('cymolthemes_number_of_posts_on_pcat') ){
function cymolthemes_number_of_posts_on_pcat( $query ){
	$pfcat_show = cymolthemes_get_option('pfcat_show');
	$pfcat_show = ( !empty($pfcat_show) ) ? esc_attr( $pfcat_show ) : '9' ;

	if( is_tax( 'cmt_portfolio_category' ) && $query->is_main_query() ){
		$query->set('posts_per_page', $pfcat_show);
	}
	return $query;
}
}
add_filter('pre_get_posts', 'cymolthemes_number_of_posts_on_pcat');


/**
 * Setting limit to show number of Portfolios on Portfolio Category page
 */
if( !function_exists('cymolthemes_number_of_posts_on_teamgroup') ){
function cymolthemes_number_of_posts_on_teamgroup( $query ){
	$teamcat_show = cymolthemes_get_option('teamcat_show');
	$teamcat_show = ( !empty($teamcat_show) ) ? esc_attr( $teamcat_show ) : '9' ;
	
	if( is_tax( 'cmt_team_group' ) && $query->is_main_query() ){
		$query->set('posts_per_page', $teamcat_show);
	}
	return $query;
}
}
add_filter('pre_get_posts', 'cymolthemes_number_of_posts_on_teamgroup');


/**
 *  Search results page setup
 */
if( !function_exists('cymolthemes_search_filter') ){
function cymolthemes_search_filter( $query ) {
...
add_filter('pre_get_posts','cymolthemes_search_filter');
...
/** Post Like ajax **/
add_action('wp_ajax_cymolthemes-portfolio-likes', 'cymolthemes_ajax_callback' );
add_action('wp_ajax_nopriv_cymolthemes-portfolio-likes', 'cymolthemes_ajax_callback' );

function cymolthemes_ajax_callback(){
	if(isset($_POST['likes_id'])){
		$post_id = str_replace('pid-', '', $_POST['likes_id']);
		echo cymolthemes_update_like( $post_id );
	}
	exit;
}


function cymolthemes_update_like( $post_id ){
	if(!is_numeric($post_id)) return;

	$return = '';
	$likes = get_post_meta($post_id, 'cymolthemes_likes', true);
	if(!$likes){ $likes = 0; }
	$likes++;
	update_post_meta($post_id, 'cymolthemes_likes', $likes);
	setcookie('cymolthemes_likes_'.esc_attr($post_id), esc_attr($post_id), time()*20, '/');
 ...
 */
function cymolthemes_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'cymolthemes_pingback_header' );
