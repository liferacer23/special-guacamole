...
if( is_page() ){
	...
	$sidebar_right_page = get_post_meta( get_the_ID(), '_cymolthemes_metabox_sidebar', true );
...
} elseif( is_singular('cmt_portfolio') ) {  // Portfolio
...
} elseif( is_singular('cmt_service') ) {  // Service
...
} elseif( is_tax('cmt_portfolio_category') ) {  // Portfolio category
	$sidebar_right      = 'sidebar-right-portfoliocat';
	


} elseif( is_tax('cmt_service_category') ) {  // Service category
	$sidebar_right      = 'sidebar-right-servicecat';
	

} elseif( is_singular('cmt_team_member') ) {  // Team member
...
} elseif( is_tax('cmt_team_group') ) {  // Team Member Group
	...
	if( $post_id ){
		$sidebar_right_page = get_post_meta( $post_id, '_cymolthemes_metabox_sidebar', true );
		if( !empty($sidebar_right_page['right_sidebar']) ){ $sidebar_right = $sidebar_right_page['right_sidebar']; }
	}
	...
	$sidebar_right_blog = get_post_meta( $pageid ,'_cymolthemes_'.$postType.'_options_rightsidebar',true);
	...
	$sidebar_right = esc_attr($duplexo_theme_options['sidebar_post']); // Global settings
	$sidebar_right = 'sidebar-right-blog';
	$sidebar_right_post = get_post_meta($post->ID,'_cymolthemes_post_options_rightsidebar',true);
