...
if( is_page() ){
	...
	$sidebar_right_page = get_post_meta($post->ID,'_cymolthemes_page_options_rightsidebar',true);
	...
	if( function_exists('tribe_is_upcoming') ){
		if (get_post_type()=='tribe_events'){
			$events_sidebar = !empty($duplexo_theme_options['sidebar_events']) ? esc_attr($duplexo_theme_options['sidebar_events']) : 'no' ; // Global settings
	...
	if( function_exists('tribe_is_upcoming') ){
		if ( get_post_type() == 'tribe_events' || tribe_is_upcoming() || tribe_is_month() || tribe_is_by_date() || tribe_is_day() || is_single('tribe_events')){
			$events_sidebar = ( isset($duplexo_theme_options['sidebar_events']) && trim($duplexo_theme_options['sidebar_events'])!='' ) ? esc_attr($duplexo_theme_options['sidebar_events']) : 'no' ; // Global settings
			if( $events_sidebar=='right' ){
				$sidebar_right = 'sidebar-events';
			}
		}
